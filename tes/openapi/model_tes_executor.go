...
// AssertTesExecutorRequired checks if the required fields are not zero-ed
func AssertTesExecutorRequired(obj TesExecutor) error {
...
// AssertRecurseTesExecutorRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesExecutor (e.g. [][]TesExecutor), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesExecutorRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesExecutor, ok := obj.(TesExecutor)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesExecutorRequired(aTesExecutor)
	})
}
