...
// AssertTesFileTypeRequired checks if the required fields are not zero-ed
func AssertTesFileTypeRequired(obj TesFileType) error {
	return nil
}

// AssertRecurseTesFileTypeRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesFileType (e.g. [][]TesFileType), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesFileTypeRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesFileType, ok := obj.(TesFileType)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesFileTypeRequired(aTesFileType)
	})
}
