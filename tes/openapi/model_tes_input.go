...
// AssertTesInputRequired checks if the required fields are not zero-ed
func AssertTesInputRequired(obj TesInput) error {
...
// AssertRecurseTesInputRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesInput (e.g. [][]TesInput), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesInputRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesInput, ok := obj.(TesInput)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesInputRequired(aTesInput)
	})
}
