...
type TesOutput struct {
	...
	Type TesFileType `json:"type"`
}

// AssertTesOutputRequired checks if the required fields are not zero-ed
func AssertTesOutputRequired(obj TesOutput) error {
...
// AssertRecurseTesOutputRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesOutput (e.g. [][]TesOutput), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesOutputRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesOutput, ok := obj.(TesOutput)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesOutputRequired(aTesOutput)
	})
}
