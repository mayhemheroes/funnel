...
// AssertTesServiceTypeRequired checks if the required fields are not zero-ed
func AssertTesServiceTypeRequired(obj TesServiceType) error {
...
// AssertRecurseTesServiceTypeRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesServiceType (e.g. [][]TesServiceType), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesServiceTypeRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesServiceType, ok := obj.(TesServiceType)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesServiceTypeRequired(aTesServiceType)
	})
}
