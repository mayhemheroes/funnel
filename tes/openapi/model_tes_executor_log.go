...
// AssertTesExecutorLogRequired checks if the required fields are not zero-ed
func AssertTesExecutorLogRequired(obj TesExecutorLog) error {
...
// AssertRecurseTesExecutorLogRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesExecutorLog (e.g. [][]TesExecutorLog), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesExecutorLogRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesExecutorLog, ok := obj.(TesExecutorLog)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesExecutorLogRequired(aTesExecutorLog)
	})
}
