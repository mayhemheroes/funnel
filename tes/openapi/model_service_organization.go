...
// AssertRecurseServiceOrganizationRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of ServiceOrganization (e.g. [][]ServiceOrganization), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseServiceOrganizationRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aServiceOrganization, ok := obj.(ServiceOrganization)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertServiceOrganizationRequired(aServiceOrganization)
	})
}
