...
// AssertTesStateRequired checks if the required fields are not zero-ed
func AssertTesStateRequired(obj TesState) error {
	return nil
}

// AssertRecurseTesStateRequired recursively checks if required fields are not zero-ed in a nested slice.
// Accepts only nested slice of TesState (e.g. [][]TesState), otherwise ErrTypeAssertionError is thrown.
func AssertRecurseTesStateRequired(objSlice interface{}) error {
	return AssertRecurseInterfaceRequired(objSlice, func(obj interface{}) error {
		aTesState, ok := obj.(TesState)
		if !ok {
			return ErrTypeAssertionError
		}
		return AssertTesStateRequired(aTesState)
	})
}
